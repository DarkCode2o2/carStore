...
@section('path') {{ Breadcrumbs::render('showCar', $car)}} @endsection
...
@section('content')
    <div class="show-car w-full mx-auto px-4">
        <div class="mx-auto flex lg:flex-row flex-col justify-center gap-4 lg:w-11/12">
            <div class="car-info bg-slate-700 lg:w-[60%] w-full shadow-md rounded-md md:pt-10 p-0" data-aos="fade-left"  data-aos-duration="1500">
                        ...
                        <ul class="splide__list h-[300px]">
                            @foreach ($images as $image)
                                <li class="splide__slide w-1/2">
                                    <img src="{{asset("uploads/car_images/$image->image_path")}}" alt="صورة السيارة" class="w-full mx-auto md:h-[400px] h-full max-h-full max-w-full md:object-contain rounded-md">
                                </li>
                            @endforeach
                            <li class="splide__slide w-1/2">
                                <img src="{{asset("uploads/car_logos/$car->car_logo")}}" alt="صورة السيارة" class="w-full mx-auto md:h-[400px] h-full max-h-full max-w-full md:object-contain rounded-md">
                            </li>
                        </ul>
                    </div>
                </div>
                ...
                <div id="thumbnails" class="thumbnails w-full whitespace-nowrap grid auto-cols-max grid-flow-col justify-center px-1 py-2 gap-1 overflow-x-auto over scroll-smooth scrollbar-hide sm:scrollbar-default  scroll-px-2">
                    @foreach ($images as $image)
                        <img src="{{asset("uploads/car_images/$image->image_path")}}" alt="صور السيارة" class="thumbnail w-20 opacity-50 cursor-pointer m-1  object-cover rounded-md selection: h-20 max-w-full">
                    @endforeach
                    <img src="{{asset("uploads/car_logos/$car->car_logo")}}" alt="صور السيارة" class="thumbnail w-20 opacity-50 cursor-pointer m-1  object-cover rounded-md selection: h-20 max-w-full">
                </div>
            </div>
            <div class="bg-white p-4 lg:w-[40%] w-full rounded-md  shadow  mt-10 md:mt-0" data-aos="fade-left"  data-aos-duration="1500">
                ...
                <div class="flex lg:justify-between md:justify-around md:flex-row flex-col gap-5 md:items-center">
                    ...
                    <ul>
                        <li class="mb-10 border-b-2 border-gray-200 pb-2">
                            ...
                            العداد:
                            <span class="font-semibold">{{$car->car_odometer}}</span>
                        </li>
                        ...
                        <li class="mb-10 border-b-2 border-gray-200 pb-2">
                            ...
                            السعر: 
                            <span class="font-semibold">{{number_format($car->car_price)}}</span>
                        </li>

                        <li class="mb-10 border-b-2 border-gray-200 pb-2">
                            <i class="fa-solid fa-money-bill-wave ms-1 text-main-back"></i>
                            باقي التكاليف: 
                            <span class="font-semibold">{{number_format($car->car_costs)}}</span>
                        </li>

                        <li class="mb-10 border-b-2 border-gray-200 pb-2">
                            <i class="fa-solid fa-tag ms-1 text-main-back"></i>
                            الإجمالي: 
                            <span class="font-semibold">{{number_format(intval(str_replace(',', '', $car->car_costs)) + intval(str_replace(',', '', $car->car_price)))}}</span>
                        </li>
                    </ul>
                </div>
                <div class="mb-10 border-b-2 border-gray-200 pb-2 md:text-center">
                    ...
                    الموقع: 
                    <span class="font-semibold">{{$car->car_location}}</span>
                </div>
        ...
        <div class="car-description w-full  my-20 lg:w-11/12 max-w-full mx-auto bg-teal-500 rounded-md p-4 shadow">
            ...
            @php
                ...
                foreach ($text as $line) {
                        echo "<span class='block mb-5'><i class='fa-solid fa-circle text-main-text mx-2 text-sm'></i>$line</span>";
                }
            @endphp
        </div>
        @if ($car->car_status)
