...
@section('content')
        ...
        <div class="create_car pt-20 pb-96 sm:px-0 px-2">
            <div class="max-w-6xl mx-auto mt-11">
                <form action="/manageCar" class="myForm" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="border-b-2 border-gray-300 pb-8 mb-8">
                        <div class="md:w-1/3 w-full bg-white h-60 relative rounded-md flex overflow-hidden shadow-sm">
                            <img id="car_logo" class="object-cover">
                            <h1 id="displayMain" class="absolute text-2xl left-1/2 top-1/2 text-blue-600 -translate-y-1/2 -translate-x-1/2">معاينة الصورة</h1>
                        </div>
                        <div class="flex mb-4 justify-between sm:flex-row flex-col gap-2">
                            ...
                            <div class="sm:w-1/2">
                                ...
                                <input type="text"
                                    class="rounded border-none shadow w-full text-md max-w-full"
                                    name="car_company"
                                    placeholder="مثال: هيونداى"
                                    value="{{old('car_company')}}"
                                    required
                                    >
                                @error('car_company')
                                    <span class="text-red-500 block my-2 text-right" style="direction: ltr">{{$message}}</span>
                    ...
                    <div class="border-b-2 border-gray-300 pb-8 mb-8">
                        ...
                        <div class="flex justify-between sm:flex-row flex-col mt-4 gap-2 sm:items-start">
                            <div class="sm:w-1/2">
                                ...
                                <textarea 
                                class="rounded border-none shadow w-full text-md max-w-full h-52"
                                name="car_description"
                                placeholder="انزل خطوة بعد كل وصف
...
...
...
..." required>{{old('car_description')}}</textarea>
                        ...
                        <div class="flex justify-center">                           
                            <div class="flex items-center justify-center  sm:w-1/2 my-4">
                                <label for="dropzone-file" class="flex flex-col items-center transition justify-center w-full p-2  rounded-lg cursor-pointer bg-blue-500 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-blue-700 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                                    ...
                                    <input id="dropzone-file" type="file" name="car_images[]" multiple accept="image/*" onchange="displayImages(this)" class="hidden image-input"/>
                                </label>
                            </div> 
                        </div>
                        @error('car_images.*')
